...
const emit = defineEmits<{
  (e: 'cancel'): void,
  (e: 'confirm', category_id: number): void
}>();
...
const confirmDelete = async () => {
  ...
  const { data: result, error } = await useFetchAPI(`/admin/categories/${props.category.category_id}/delete`, {
    method: "DELETE"
  })

  if (result.value) {
    isDeleting.value = false;
    emit('confirm', props.category.category_id)
  }
